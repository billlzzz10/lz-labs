...
-   [ ] **Collaboration Features:** (พิจารณา) ฟีเจอร์สำหรับการทำงานร่วมกันบนโปรเจกต์
...
4.  **เตรียมพร้อมสำหรับ Backend (Service Layer):**
    *   **สร้าง Service Layer:** สร้างโฟลเดอร์ `services/` ขึ้นมา เพื่อแยก Logic ที่เกี่ยวกับการจัดการข้อมูล (ปัจจุบันคือ `localStorage`) ออกจากคอมโพเนนต์
    *   **ตัวอย่าง:** สร้าง `DataService.ts` ที่มีฟังก์ชันอย่าง `getNotes(projectId)`, `saveNote(noteData)` ซึ่งเบื้องหลังจะทำงานกับ `localStorage` แต่ในอนาคตสามารถสลับไปเรียกใช้ API ของ Backend จริงได้โดยแก้ไขแค่ในไฟล์นี้ที่เดียว
